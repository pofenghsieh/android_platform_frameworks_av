...
WifiDisplaySink::WifiDisplaySink(
    ...
    mSupportedSinkVideoFormats.disableAll();

    mSupportedSinkVideoFormats.setNativeResolution(
...
void WifiDisplaySink::onGetParameterRequest(
    ...
    if (strstr(content, "wfd_video_formats\r\n") != NULL) {
        ...
        body.append(mSupportedSinkVideoFormats.getFormatSpec(false /* forM4Message */));
        body.append("\r\n");
    }
    ...
    if (strstr(content, "wfd_audio_codecs\r\n") != NULL) {
        ...
        body.append(
                StringPrintf("%s\r\n",
                                ("AAC 0000000F 00"))); // 2 ch AAC 48KHz
        body.append("\r\n");
    }
...
status_t WifiDisplaySink::sendSetup(int32_t sessionID, const char *uri) {
    mRTPSink = new RTPSink(mNetSession, mProducer);
...
void WifiDisplaySink::onSetParameterRequest(
    ...
    if (!parameter.empty()) {
        if (parameter == "none") {
            ...
            if (!mSourceRequestedVideoFormats.parseFormatSpec(parameter.c_str())) {
