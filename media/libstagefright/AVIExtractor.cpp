...
#ifdef OMAP_ENHANCEMENT
    /* If type == 'pads', just ignore it */
    if (type == FOURCC('p', 'a', 'd', 's')) {
        ALOGV("Skipped 'pads' tag, offset %llu", offset);
        return OK;
    }

    uint32_t scale = U32LE_AT(&data[20]);
    uint32_t rate = U32LE_AT(&data[24]);
    uint32_t fps = rate / scale;
    //uint32_t fps = (scale * 1.0 / rate) + 0.5;

    if (!(scale && rate)) {
        ALOGE("Scale/Rate is invalid (%u/%u). "
              "This file has been generated by broken software."
              "Setting to default 1/25", rate, scale);
...
status_t AVIExtractor::getSampleIndexAtTime(
    ...
    if (track.mBytesPerSample > 0) {
#ifdef OMAP_ENHANCEMENT
        uint64_t bpsec = (track.mRate * track.mScale) * track.mBytesPerSample;
        uint64_t bpusec = bpsec / 1000000ll;
        size_t closestByteOffset = bpusec * timeUs;
#else
